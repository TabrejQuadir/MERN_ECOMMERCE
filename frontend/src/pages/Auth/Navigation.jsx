    ...
    <div id="navigation-container" className="flex flex-col justify-between p-4 text-white bg-black">
      ...
      <div className="relative">
        <button onClick={toggleDropdown} className="flex items-center text-gray-800 focus:outline-none">
          {userInfo ? <span className="-ml-3 text-black font-bold">{userInfo.username}</span> : <></>}
          {userInfo && (
            <svg
        ...
        {dropdownOpen && userInfo && (
          <ul
            className={`absolute right-0  space-y-2  text-white bg-black  ${
              !userInfo.isAdmin ? "bottom-20" : "bottom-8"
            }`}
          >
            {userInfo.isAdmin && (
              <>
            ...
            <li className=" hover:font-bold">
              <button onClick={logoutHandler} className="block w-full px-4 py-2 text-left ">
                Logout
              </button>
            </li>
          </ul>
